...
object MyReaders {
  ...
  def emailUser(username: String, userEmail: String): String = {
    val emailHandler: Reader[Configuration, EmailService] = Reader(conf => EmailService(conf.emailReplyTo))
    (for {
      service <- emailHandler
    } yield service.sendEmail(userEmail, getLastOrderStatus(username))).run(config)
  }
