package part5alien.mysolutions

import cats.Monoid

object MyInvariantFunctors {
  ...
  implicit val optionStringCrypto: Crypto[Option[String]] = caesarCypher.imap(x => x.getOrElse(""), y => Option(y))

  // TODO 2 - if you have a Crypto[T] => Crypto[Option[T]] and if you have a Monoid[T] in scope
  implicit def optionCrypto[T](implicit crypto: Crypto[T], monoid: Monoid[T]): Crypto[Option[T]] =
    crypto.imap(ot => ot.getOrElse(monoid.empty), t => Option(t))
